...
class FLC : public cSimpleModule
{
  ...
  private:
	int init_contor=0; /* used in the beginning to get all the network loads */
	...
	int delta_f; /* delta read from the xml file */
...
void FLC::initialize()
{
   ...
   current = rootelement->getElementByPath("./delta");
   delta_f = atoi(current->getFirstChild()->getNodeValue());
   ev<< "delta:" << delta_f << ".\n";
...
{
    for(int i = 0; i < networkSize;i++){
        if (msg->arrivedOn("rxServer",i))
        {
            networkLoad[i] = (double)msg->par("netLoad");
            transferRates[i] = (double)msg->par("transferRate");
            break;
    ...
    if(init_contor < networkSize){
        ...
        job->addPar("select_network");
        job->par("select_network") = bestNetwork;
        send(job,"txServer");
